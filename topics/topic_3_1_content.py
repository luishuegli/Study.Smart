...
content_3_1 = {
    ...
    "anchor": {
        ...
        "text": {"de": "Stell dir vor, du läufst einen Weg von links nach rechts entlang. Bei jedem Schritt sammelst du die Wahrscheinlichkeit auf, an diesem Punkt (oder davor) zu sein. Die Verteilungsfunktion ist dein 'Rucksack', der immer voller wird, bis du am Ende 100% (1.0) gesammelt hast.", "en": "Imagine walking along a path from left to right. At each step, you collect the probability of being at that point (or before it). The distribution function is your 'backpack' that gets fuller and fuller until you have collected 100% (1.0) at the end."},
        "metaphor": {"de": "Der Wahrscheinlichkeits-Scanner", "en": "The Probability Scanner"}
    },
    "theory": {
        "def_title": {"de": "Definition", "en": "Definition"},
        "def_text": {"de": "Die kumulative Verteilungsfunktion (CDF) gibt an, wie viel Wahrscheinlichkeit sich bis zu einem Punkt x angesammelt hat.", "en": "The Cumulative Distribution Function (CDF) tells you how much probability has accumulated up to a point x."},
        "formula": r"F(x) = P(X \leq x)",
        "props": {
            "p1": {"de": "Beginnt bei 0 (links) und endet bei 1 (rechts).", "en": "Starts at 0 (left) and ends at 1 (right)."},
            "p2": {"de": "Ist monoton steigend (man kann Wahrscheinlichkeit nicht verlieren).", "en": "Is monotonically increasing (you cannot lose probability)."}
        }
    },
    "interactive": {
    ...
    "pro_tip": {
        "de": "Um die Wahrscheinlichkeit für einen Bereich (z.B. zwischen a und b) zu finden, subtrahiere einfach: P(a < X ≤ b) = F(b) - F(a). Das ist viel einfacher als Integrieren!",
        "en": "To find the probability for a range (e.g., between a and b), simply subtract: P(a < X ≤ b) = F(b) - F(a). That's much easier than integrating!"
    }
}
...
def render_subtopic_3_1(model):
    ...
    with c2:
        with st.container(border=True):
            st.markdown(f"**{t({'de': 'Der Zusammenhang', 'en': 'The Connection'})}**")
            st.caption(t({"de": "Die Dichte (PDF) zeigt die Wahrscheinlichkeit an einem Punkt. Die Verteilung (CDF) ist die Summe bis dahin.", "en": "The Density (PDF) shows probability at a point. The Distribution (CDF) is the sum up to that point."}))
            # Simple SVG Illustration
            st.markdown(f"""
            <div style="text-align: center; margin-top: 10px;">
                {render_icon("layers", size=48, color="#007AFF")}
                <div style="font-size: 12px; margin-top: 8px;">PDF $\\to$ Integration $\\to$ CDF</div>
            </div>
            """, unsafe_allow_html=True)
