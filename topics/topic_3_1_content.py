...
def render_subtopic_3_1(model):
    ...
    with st.container(border=True):
        ...
        with col_ctrl:
            ...
            st.metric("Position x", f"{x_val:.1f}")
            st.metric(f"F(x) = P(X ≤ {x_val:.1f})", f"{cdf_val:.1%}")

            st.markdown(f"""
            <div style="margin-top: 20px; font-size: 0.9em; color: #666;">
            We move to <b>{x_val}</b>.<br>
            We have collected <b>{cdf_val:.1%}</b> of the total area.
            </div>
            """, unsafe_allow_html=True)
    ...
    with st.container(border=True):
        ...
        with col_m2:
            ...
            st.metric("Current Level", f"{current_prob:.1%}", delta=f"{delta*100:.1f}%", delta_color="inverse")
